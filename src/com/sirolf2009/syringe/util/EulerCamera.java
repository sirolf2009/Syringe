...
public final class EulerCamera implements Camera {
	...
	public void processMouse(float mouseSpeed, float maxLookUp, float maxLookDown) {
		...
		if(Syringe.player != null) {
			//Syringe.player.setRotX(pitch);
			Syringe.player.setRotY(180-yaw);
			//Syringe.player.setRotZ(roll);
		}
	}
	...
	public void moveFromLook(float dx, float dy, float dz) {
		if(Syringe.player != null) {
			Syringe.player.setPosX(Syringe.player.getPosX() - (float) (dx * (float) sin(toRadians(yaw - 90)) + dz * sin(toRadians(yaw))));
			//Syringe.player.setPosY(Syringe.player.getPosY() + (float) (dx * (float) sin(toRadians(yaw - 90)) + dz * sin(toRadians(yaw))));
			Syringe.player.setPosZ(Syringe.player.getPosZ() + (float) (dx * (float) cos(toRadians(yaw - 90)) + dz * cos(toRadians(yaw))));
		} else {
